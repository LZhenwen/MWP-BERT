...
def prepare_data(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, tree=False):
    ...
    for pair in pairs_trained:
        for idx in range(len(pair[0])):
            if pair[0][idx] == 'NUM':
                pair[0][idx] = 'n'
            pair[0][idx] = pair[0][idx].lower()
        #print(pair[0])
        #input_seq = ['[CLS]'] + pair[0]
        num_stack = []
        for word in pair[1]:
            temp_num = []
            flag_not = True
        ...
        for idx,i in enumerate(inputs['input_ids'].squeeze()):
            if tokenizer.convert_ids_to_tokens(int(i)) == 'Ġn' or tokenizer.convert_ids_to_tokens(int(i)) == 'n':
                num_pos.append(idx)
        num_stack.reverse()
...
def prepare_train_batch(pairs_to_batch, batch_size):
    ...
    return input_batches, input_lengths, output_batches, output_lengths, nums_batches, num_stack_batches, num_pos_batches, num_size_batches, num_value_batches, graph_batches, bert_input_batches
...
def prepare_data_pretraining_all(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, tree=False):
    input_lang = Lang()
    output_lang = Lang()
    train_pairs = []
    test_pairs = []
