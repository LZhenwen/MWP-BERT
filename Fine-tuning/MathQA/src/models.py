...
from transformers import BertTokenizer, BertModel, BertForMaskedLM, BertConfig, WEIGHTS_NAME, CONFIG_NAME, AutoModelWithLMHead, AutoModel
import os
...
class EncoderBert(nn.Module):
    def __init__(self, hidden_size, auto_transformer=False, bert_pretrain_path='', dropout=0.5):
        super(EncoderBert, self).__init__()
        self.bert_model = None
        self.embedding_size = 768
        if auto_transformer:
            print("auto_model: ", bert_pretrain_path)
            self.bert_model = AutoModel.from_pretrained(bert_pretrain_path)
            self.embedding_size = self.bert_model.config.d_model
        else:
    ...
    def forward(self, input_ids, attention_mask=None, analyze=False):
        # Note: we run this all at once (over multiple batches of multiple sequences)
        ...
        if analyze:
            return pade_outputs, problem_output, output[2]
        else:
            return pade_outputs, problem_output
