...
def train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               encoder, predict, generate, merge, output_lang, num_pos, english=False):
...
def contra_loss_std(input: torch.Tensor, target: torch.Tensor, tau: float=5e-2)->torch.Tensor:
    batch_emb = torch.empty_like(input)
    batch_emb = torch.cat([batch_emb, batch_emb], 0)
    batch_emb[0::2] = input
    batch_emb[1::2] = target
    ...
    sim_score = sim_score / tau
    loss = nn.CrossEntropyLoss()(sim_score, y_true)
    return loss

def contra_train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
                      encoder, predict, generate, merge,
                      output_lang, num_pos, args, alpha, contra_loss_func="", subtree_pos_pair=None):
    encoder.train()
    predict.train()
    generate.train()
    merge.train()
    ...
    problem_output_ls = []
    total_loss = None
    for nd in range(len(input_batch)):
        no_expr_loss = False
        if args.neg_no_expr_loss and nd >= 2:
            no_expr_loss = True
        all_node_outputs, target, target_length_out, problem_output, q_list = train_tree_core(
                    input_batch[nd], input_length[nd], target_batch[nd], target_length[nd],
                    nums_stack_batch[nd], num_size_batch[nd], generate_nums, encoder, predict, generate, merge,
                    output_lang, num_pos[nd], no_expr_loss)
        if no_expr_loss == False:
            loss = masked_cross_entropy(all_node_outputs, target, target_length_out)
            if total_loss == None:
                total_loss = loss
            else:
        ...
        if args.contra_common_tree_pair:
            problem_batch = []
            for i in range(len(subtree_pos_pair)):
                pos = subtree_pos_pair[i][nd] if nd < len(subtree_pos_pair[i]) else 0
                problem_batch.append(q_list[pos][i,:,:])
            problem_batch = torch.cat(problem_batch, dim=0)
            problem_output_ls.append(problem_batch)
        else:
            problem_output_ls.append(problem_output)
    if contra_loss_func == "":
        if len(input_batch) > 2:
            pos_sim = torch.cosine_similarity(problem_output_ls[0], problem_output_ls[1], dim=1).unsqueeze(1)
            neg_sim = []
            for nd in range(2, len(input_batch)):
                neg_sim.append(torch.cosine_similarity(problem_output_ls[0], problem_output_ls[nd], dim=1).unsqueeze(1))
            sim_list = [pos_sim] + neg_sim
            sim = torch.cat(sim_list, dim=1) # B * (1+num_neg)
            y_target = [0 for _ in range(sim.shape[0])]
            y_target = torch.LongTensor(y_target)
            if USE_CUDA:
                y_target = y_target.cuda()
                
            contra_loss_func = torch.nn.CrossEntropyLoss()
            contra_loss = contra_loss_func(sim, y_target)
        else:
            contra_loss = contra_loss_std(problem_output_ls[0], problem_output_ls[1]).mean()
    elif contra_loss_func == "margin":
        pos_sim = torch.cosine_similarity(problem_output_ls[0], problem_output_ls[1], dim=1)
        neg_sim = torch.cosine_similarity(problem_output_ls[0], problem_output_ls[2], dim=1)
        contra_loss_func = torch.nn.MarginRankingLoss(args.contra_loss_margin)
        target = [1 for _ in range(pos_sim.shape[0])]
        target = torch.LongTensor(target)
        if USE_CUDA:
            target = target.cuda()
        contra_loss = contra_loss_func(pos_sim, neg_sim, target)

    e_loss_value = total_loss.item()
    c_loss_value = contra_loss.item()
    if alpha > 1e-6:
        final_loss = total_loss + alpha * contra_loss
        final_loss.backward()
    else:
        total_loss.backward()

    return e_loss_value + c_loss_value, c_loss_value, e_loss_value
