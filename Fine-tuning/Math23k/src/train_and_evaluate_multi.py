...
class Joint(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size, n_layers, op_nums, input_size_predict,lr,wd):
        super(Joint, self).__init__()

        self.encoder = EncoderSeq(input_size=input_size, embedding_size=embedding_size, hidden_size=hidden_size,
                            n_layers=n_layers)
        self.predict = Prediction(hidden_size=hidden_size, op_nums=op_nums,
                            input_size=input_size_predict)
        self.generate = GenerateNode(hidden_size=hidden_size, op_nums=op_nums,
                                embedding_size=embedding_size)
        self.merge = Merge(hidden_size=hidden_size, embedding_size=embedding_size)

        self.encoder = nn.DataParallel(self.encoder)
        self.predict = nn.DataParallel(self.predict)
        self.generate = nn.DataParallel(self.generate)
        self.merge = nn.DataParallel(self.merge)

        self.encoder_optimizer = torch.optim.Adam(self.encoder.parameters(), lr=lr, weight_decay=wd)
        self.predict_optimizer = torch.optim.Adam(self.predict.parameters(), lr=lr, weight_decay=wd)
        self.generate_optimizer = torch.optim.Adam(self.generate.parameters(), lr=lr, weight_decay=wd)
        self.merge_optimizer = torch.optim.Adam(self.merge.parameters(), lr=lr, weight_decay=wd)

        self.encoder_scheduler = torch.optim.lr_scheduler.StepLR(self.encoder_optimizer, step_size=16000, gamma=0.5)
        self.predict_scheduler = torch.optim.lr_scheduler.StepLR(self.predict_optimizer, step_size=16000, gamma=0.5)
        self.generate_scheduler = torch.optim.lr_scheduler.StepLR(self.generate_optimizer, step_size=16000, gamma=0.5)
        self.merge_scheduler = torch.optim.lr_scheduler.StepLR(self.merge_optimizer, step_size=16000, gamma=0.5)
    ...
    def train_tree(self, input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
                output_lang, num_pos, bert_input, english=False):
        # sequence mask for attention
        seq_mask = []
        max_len = max(input_length)
        for i in input_length:
        ...
        padding_hidden = torch.FloatTensor([0.0 for _ in range(self.predict.module.hidden_size)]).unsqueeze(0)
        batch_size = len(input_length)
        ...
        encoder_outputs, problem_output = self.encoder(input_var, input_length, bert_input)
        # Prepare input and output variables
        node_stacks = [[TreeNode(_)] for _ in problem_output.split(1, dim=0)]
        ...
        num_size = max(copy_num_len)
        all_nums_encoder_outputs = get_all_number_encoder_outputs(encoder_outputs, num_pos, batch_size, num_size,
                                                                self.encoder.module.hidden_size)
        ...
        torch.nn.utils.clip_grad_norm_(self.predict.parameters(), 5)
        torch.nn.utils.clip_grad_norm_(self.generate.parameters(), 5)
        torch.nn.utils.clip_grad_norm_(self.merge.parameters(), 5)
        ...
        return loss.item()
    def evaluate_tree(self, input_batch, input_length, generate_nums, output_lang, num_pos,
                  bert_input, beam_size=5, english=False, max_length=MAX_OUTPUT_LENGTH):
        ...
        encoder_outputs, problem_output = self.encoder.evaluate(input_var, [input_length], bert_input)
